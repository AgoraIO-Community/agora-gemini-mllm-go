...
import { DEMO_MODELS, type DemoModel, type ThinkingLevel } from "@/components/ModelChoice";
...
export default function LandingPage() {
	...
	const [model, setModel] = useState<DemoModel>(DEMO_MODELS.live);
		...
		try {
			...
			const [agentIdResult, rtm] = await Promise.all([
				startAgent(
					...
					Number(config.uid),
					{ model, ...(model === DEMO_MODELS.extendedThinking ? { thinkingLevel } : {}) },
				).catch((err) => {
	...
	const handleTokenWillExpire = useCallback(
		async (uid: string): Promise<AgoraRenewalTokens> => {
			try {
				...
				// The backend get_config issues RTM-capable tokens for the configured account,
