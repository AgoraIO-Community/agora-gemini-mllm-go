...
async function verifyRewriteContract() {
  ...
  try {
    ...
    assert(
      ...
      'next.config.ts should rewrite /api/get_config to /get_config on the Go backend',
    )
    assert(
      rewrites.some(
        (rewrite) => rewrite.source === '/api/startAgent' && rewrite.destination === 'http://localhost:8000/startAgent',
      ),
      'next.config.ts should rewrite /api/startAgent to /startAgent on the Go backend',
    )
    assert(
      rewrites.some(
        (rewrite) => rewrite.source === '/api/stopAgent' && rewrite.destination === 'http://localhost:8000/stopAgent',
      ),
      'next.config.ts should rewrite /api/stopAgent to /stopAgent on the Go backend',
    )
  } finally {
...
async function verifyApiClientRequests() {
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (url.pathname === '/api/startAgent') {
      ...
      assert(body.model === 'models/gemini-3.8-live-extended-thinking', 'POST /api/startAgent should include the public 3.8 model ID')
      assert(body.thinkingLevel === 'high', 'POST /api/startAgent should include the selected thinking level')
  ...
  try {
    ...
    const agentId = await startAgent('test-channel', 9999, 1234, { model: 'models/gemini-3.8-live-extended-thinking', thinkingLevel: 'high' })
